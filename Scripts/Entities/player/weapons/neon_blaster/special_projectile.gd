...
@export var _tick_burst_projectiles: GPUParticles2D
...
func _process(delta: float) -> void:
	...
	_c_neon_sprite_lite.rotate(_sprite_rotation_speed * delta)

	var sine_wave: float = (sin(_sprite_flash_frequency * Time.get_ticks_msec() / 1000.0) + 1) / 2
	_c_neon_sprite_lite.alpha_multiplier = 0.2 + _sprite_flash_amplitude * sine_wave
